...
export default function RegionPage() {
  ...
  const regionSlugMatch = regions.find(
    ...
    async function fetchCategoryProducts() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/products?region=${encodeURIComponent(region)}&limit=100`,
        );
        if (res.ok) {
        ...
        <div className="container mx-auto px-4 py-6 md:py-8">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            ...
            {categories.map((sub) => (
              <Button
                key={sub.id}
                variant={filters.categoryId === sub.id ? "default" : "outline"}
                size="sm"
                className={cn(
                  themeId === "kilimanjaro" &&
                ...
                onClick={() => setFilters({ ...filters, categoryId: sub.id })}
              >
                {sub.name}
              </Button>
            ))}
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <CardContent className="space-y-1">
          <Button
            variant={filters.categoryId === null ? "secondary" : "ghost"}
            size="sm"
          ...
          {categories.map((sub) => (
            <Button
              key={sub.id}
              variant={filters.categoryId === sub.id ? "secondary" : "ghost"}
              size="sm"
              className="w-full justify-start"
              onClick={() => setFilters({ ...filters, categoryId: sub.id })}
            >
